...
with open('keywordsCity.txt','r') as keywords:
    for kw in keywords:
        try:
            city, kw = kw.split('\t')
            ...
            for tr in content.table.findAll('tr')[1:]:
                ...
                fileSEO.write(city + '\t' + kw[:-1] + '\t' + rowNum + '\t' + keyword + '\t' + searches + '\t' + includes + '\t' + loc1 + '\t' + loc2 + '\t' + score + '\n')
            fileSEO.write(city + '\t' + kw[:-1] + '_sumAll\t' + str(rowSum) + '\t' + kw[:-1] + '\t' + str(searchesSum) + '\t' + str(includesSum) + '\n')
            fileSEO.write(city + '\t' + kw[:-1] + '_sum1\t' + str(score1) + '\t' + kw[:-1] + '\t' + str(searchesSum1) + '\t' + str(includesSum1) + '\n')
            fileSEO.write(city + '\t' + kw[:-1] + '_sum2\t' + str(score2) + '\t' + kw[:-1] + '\t' + str(searchesSum2) + '\t' + str(includesSum2) + '\n')
            fileSEO.write(city + '\t' + kw[:-1] + '_sum3\t' + str(score3) + '\t' + kw[:-1] + '\t' + str(searchesSum3) + '\t' + str(includesSum3) + '\n')
            time.sleep(0.1)
        except Exception,e:
            print e
            print
