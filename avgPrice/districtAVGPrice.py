...
for city in citys.keys():
    ...
    sql = "SELECT huxing_shi,AVG(price),COUNT(1) hs FROM house_source_rent WHERE post_at>=UNIX_TIMESTAMP('2013-9-29') AND huxing_shi<=3 AND huxing_shi>=1 GROUP BY huxing_shi HAVING hs>=3"
    cursor = conn.cursor()
    cursor.execute(sql)
...
for city in citys.keys():
    ...
    sql = "SELECT AVG(price),count(1) hs FROM house_source_share WHERE post_at>=UNIX_TIMESTAMP('2013-9-29') having hs>=3"
    cursor = conn.cursor()
    cursor.execute(sql)
    for rows in cursor.fetchall():
        try:
...
for city in citys.keys():
    ...
    sql = "SELECT huxing_shi,AVG(price),COUNT(1) hs FROM house_source_rent_premier WHERE city=" + city + " and post_at>=UNIX_TIMESTAMP('2013-9-29') AND huxing_shi<=3 AND huxing_shi>=1 GROUP BY huxing_shi HAVING hs>=3"
    #print sql
    cursor = conn.cursor()
    cursor.execute(sql)
    for rows in cursor.fetchall():
...
for city in citys.keys():
    ...
    sql = "SELECT AVG(price),count(1) hs FROM house_source_share_premier WHERE city=" + city + " and post_at>=UNIX_TIMESTAMP('2013-9-29') having hs>=3"
    #print sql
    cursor = conn.cursor()
    cursor.execute(sql)
    for rows in cursor.fetchall():
