...
for city in citys.keys():
    conn=MySQLdb.connect(host="192.168.116.20",user="yangyu",passwd="c1b78739d",port=3310,db=city,charset="utf8")
    sql = "SELECT district_name,street_name,huxing_shi,AVG(price),COUNT(1) hs FROM house_source_rent WHERE post_at>=UNIX_TIMESTAMP('2013-10-29') AND huxing_shi<=3 AND huxing_shi>=1 GROUP BY district_name,street_name,huxing_shi HAVING hs>=3"
    cursor = conn.cursor()
    cursor.execute(sql)
...
for city in citys.keys():
    conn=MySQLdb.connect(host="192.168.116.20",user="yangyu",passwd="c1b78739d",port=3310,db=city,charset="utf8")
    sql = "SELECT district_name,street_name,AVG(price),count(1) hs FROM house_source_share WHERE post_at>=UNIX_TIMESTAMP('2013-10-29') GROUP BY district_name,street_name having hs>=3"
    cursor = conn.cursor()
    cursor.execute(sql)
    for rows in cursor.fetchall():
        try:
...
#rent AVG_price
for city in citys.keys():
    sql = "SELECT district_name,street_name,huxing_shi,AVG(price),COUNT(1) hs FROM house_source_rent_premier WHERE city=" + city + " and post_at>=UNIX_TIMESTAMP('2013-10-29') AND huxing_shi<=3 AND huxing_shi>=1 GROUP BY district_name,street_name,huxing_shi HAVING hs>=3"
    #print sql
    cursor = conn.cursor()
    cursor.execute(sql)
    for rows in cursor.fetchall():
...
#share AVG_price
for city in citys.keys():
    sql = "SELECT district_name,street_name,AVG(price),count(1) hs FROM house_source_share_premier WHERE city=" + city + " and post_at>=UNIX_TIMESTAMP('2013-10-29') GROUP BY district_name,street_name having hs>=3"
    #print sql
    cursor = conn.cursor()
    cursor.execute(sql)
    for rows in cursor.fetchall():
