...
dtFile = open('normalDomainType','r').readlines()
for t in dtFile[1:-1]:
    dicType.append(t[:-1])
...
pinyinFile = open('letters','r').readlines()
for py in pinyinFile[1:-1]:
    ...
    for py2 in pinyinFile:
        ...
        for py3 in pinyinFile:
            ...
            totalDic.append(py+py2+py3)
'''            for py4 in pinyinFile:
                py4 = py4[:-1]
                dic[4].append(py+py2+py3+py4)
'''

result = open('lettersUnRegged','a')

totalDic = list(set(list(totalDic)))
#sys.exit()

timer=0
for dm in totalDic: 
    for dtype in dicType:
        domainName = dm + dtype
        try:
            #print domainName
