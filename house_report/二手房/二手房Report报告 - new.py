...
for d in data:
    ...
    cursor = conn.cursor()
    cursor.execute('select huxing_shi,price,area from house_source_sell_premier where house_id='+house_id+';')
...
for area in resultarea:
    a,allCount,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10 = area.split('\t')
    Reportarea.write(a+'\t'+allCount.split('-')[0]+'\t'+allCount.split('-')[1]+'\t'+c1.split('-')[0]+'\t'+c2.split('-')[0]+'\t'+c3.split('-')[0]+'\t'+c4.split('-')[0]+'\t'+c5.split('-')[0]+'\t'+c6.split('-')[0]+'\t'+c7.split('-')[0]+'\t'+c8.split('-')[0]+'\t'+c9.split('-')[0]+'\t'+c10.split('-')[0]+'\n')
Reportarea.close()
