...
#打开文件，准备写入结果数据
tmpfile = open(dateNow+'renttmpfile.txt','w')
...
for d in data:
    houseId,companyName,district,street,lp,clicks,datetime = d.split('\t')
    if houseId not in houseIds:
        houseIds[houseId] = ''
houseIds = houseIds.keys()
print len(houseIds)
#for i in range(0,7543):
    #houseIdTmp = []
for houseId in houseIds:
    #houseIdTmp.append(houseId)
    #houseId = ','.join(houseIdTmp)
    sql = 'select house_id,huxing_shi,price from house_source_rent_premier where house_id = '+houseId+';'
    cursor.execute(sql)
    rows = cursor.fetchall()
    #print len(rows)
    for row in rows:
        tmpfile.write(str(row[0])+'\t'+str(row[1])+'\t'+str(row[2])+'\n')
tmpfile.close()
cursor.close()
'''
