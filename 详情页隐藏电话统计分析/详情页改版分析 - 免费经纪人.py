...
for houseId in houseIdClick:
    ...
    cursor.execute(sql)
    for row in cursor.fetchall():
        resultClickmfzj.write(houseId+'\t'+houseIdClick[houseId])
        for i in row:
            resultClickmfzj.write('\t'+str(i))
        resultClickmfzj.write('\n')
resultClickmfzj.close()
'''
#对于免费贴展示部分的统计
for houseId in houseIdShow:
    ...
    try:
        ...
        for row in cursor.fetchall():
            resultShowmfzj.write(houseId+'\t'+houseIdShow[houseId])
            for i in row:
                resultShowmfzj.write('\t'+str(i))
            resultShowmfzj.write('\n')
    except:
        print '执行错误！'
resultShowmfzj.close()
cursor.close()
