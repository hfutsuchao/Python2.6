...
cityList = ["北京市", "上海市", "深圳市", "广州市", "成都市", "东莞市", "杭州市", "太原市", "天津市", "武汉市", "郑州市"]
...
if __name__ == '__main__':
    ...
    expandCat = {'民宅':['租房', '合租房', '二手房出售' , '新房出售'], '租赁':['租房', '合租房'], '商业':['商铺（租）' , '商铺（售）' , '写字楼（租）' , '写字楼（售）']}
    
    for cat in mainCategories:
        print cat
        dic[cat] = getUVPVPub(startDate,EndDate,cat,'全国','1',27)
    ...
    for eCat in expandCat:
        ...
        for cat in expandCat[eCat]:
            for city in dic[cat]:
                if city not in dic[eCat]:
                    dic[eCat][city] = {}
                    dic[eCat][city]['uv'] = dic[cat][city]['uv']
                    dic[eCat][city]['pv'] = dic[cat][city]['pv']
                else:
                    dic[eCat][city]['uv'] = dic[eCat][city]['uv'] + dic[cat][city]['uv']
                    dic[eCat][city]['pv'] = dic[eCat][city]['pv'] + dic[cat][city]['pv']
    
    for cat in dic:
        print cat
        wFile = open('UVPVPub/' + cat.replace('/','').decode('utf-8').encode('gbk') + '.txt','w')
        for i in sorted(dic[cat].items(), key=lambda x:x[1]['uv'], reverse=True):
            wFile.write(startDate + '~' + EndDate + '\t' + i[0] + '\t' + str(i[1]['uv']) + '\t' + str(i[1]['pv']) + '\n')