...
def getUVPVPub(StartDate,EndDate,majorCategory='全部',city='全国',source='1',days=1):
    ...
    for data in dataR['data']:
        tmpDic[data["Col2"]] = {'uv':data["Col4"],'pv':data["Col8"],'v':data["Col6"],'pub':data["Col11"]}
...
if __name__ == '__main__':
    ...
    dateList = [getLastSomeDay(1,7),getLastSomeDay(8,7),getLastSomeDayLastMonth(1,7),getLastSomeDayLastYear(1,7)]
    dic = {}
    
    #mainCategories = ['全部' , '租房' , '合租房' , '商铺（租）' , '商铺（售）' , '写字楼（租）' , '写字楼（售）' , '厂房/仓库/土地' , '二手房出售' , '新房出售' ]
    mainCategories = ['全部']
    
    #expandCat = {'民宅':['租房', '合租房', '二手房出售' , '新房出售'], '租赁':['租房', '合租房'], '商业':['商铺（租）' , '商铺（售）' , '写字楼（租）' , '写字楼（售）'] , '二手房出售':['二手房出售',] , '新房出售':['新房出售',]}
    
    for cat in mainCategories:
        #print cat
        wFile = open('UVPVPubWeeklyReport/' + cat.replace('/','').decode('utf-8').encode('gbk') + '.txt','w')
        for dateElm in dateList: 
            startDate = str(dateElm[1])
            EndDate = str(dateElm[0])
            print startDate,EndDate
            dic[cat] = getUVPVPub(startDate,EndDate,cat,'全国','1',7)
            #print json.dumps(result,indent=1)
            for i in sorted(dic[cat].items(), key=lambda x:x[1]['uv'], reverse=True):
                try:
                    #print i[0],i[0].__class__
                    if i[0] == '全国':
                        wFile.write(startDate + '~' + EndDate + '\t' + i[0] + '\t' + str(i[1]['uv']) + '\t' + str(i[1]['pv']) + '\t' + str(i[1]['pub']) + '\n')
                except Exception,e:
                    print i[0],e
                    sys.exit()
    #print getSEOUV()
