...
def getUVPVPub(days='1',city='全国',type='all',source='1'):
    global crm
    global citys
    global sourceURL
    global sources
    
    StartDate,EndDate = getDate(days)
    ...
    for data in dataR['data']:
        tmpDic[data["Col1"]+data["Col2"]] = {'uv':data["Col4"],'pv':data["Col8"],'v':data["Col6"],'pub':data["Col11"]}
...
def getUVSources(days='30',city='全国',type='uv'):
    global sources
    
    sourceDic = {}
    for s in sources:
        sourceDic[s['Text']] = {}
        sourceDic[s['Text']]['value'] = s['Value']
        sourceDic[s['Text']]['data'] = getUVPVPub(days='30',city=city,type='uv',source=sourceDic[s['Text']]['value'])
    return sourceDic

def getAllCitysSources(city='all'):
    global citys
    global cityList
    ...
    for city in cityList:
        getAVG = MyMath()
        cityDic[city] = getAVG.dicAVG(getUVSources(days='30',city=city,type='uv'))
    
    citysUV = {}
    for city in cityList:
        try:
            citysUV[city] = {}
            for source in cityDic[city]:
                citysUV[city][source] = {'uv':cityDic[city][source]['data']['datas']['uv'],'length':len(cityDic[city][source]['data']['dateList'])}
        except Exception,e:
            pass
    return citysUV
...
if __name__ == '__main__':
    print getUVPVPub()
    #print json.dumps(getUVSources(),indent=1)
    #citysUV = getAllCitysSources()
    #print json.dumps(citysUV,indent=1)
    '''for city in citysUV:
        for source in citysUV[city]:
            try:
                print city,source,citysUV[city][source]['uv'],citysUV[city][source]['length'],citysUV[city][source]['uv']/float(citysUV[city]['全流量']['uv'])
            except:
                pass'''