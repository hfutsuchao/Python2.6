...
#mdf = pd.read_csv('MSFT.csv')

def get_quota(df):
    close = df['close']
    #iv10 = mdf.set_index('Date')
    ...
    #df_all = pd.concat([df_all,iv10],axis=1)
...
def get_normlized(df,norm_type='character'):
    if norm_type == 'max':
        df_norm = df.copy()
...
def get_trade_chance(df,close,norm_type,start_date,end_date,lost):
    rate = {}
    rate['based'] = {}
    rate['based']['profit'] = {}
    buy_price = {}
    ...
    is_buy = {}
    is_sell = {}
    df_dif_norm = get_normlized(df,norm_type)
...
def main():
    ...
    #df = pd.read_sql('select * from day_k_data where code="MSFT" order by date asc;',engine)
    ...
    for code in rate:
        pass
        #print rate[code][1][1]['trade']
        #print rate[code][strategy_sum[0][0]]['trade']
