...
for code in codes:
    code = code[0]
    print code
    rate[code], profit[code] = parse(code,'2017-05-01')
    strategy[rate[code][0][0]] = 0

for code in rate:
    strategy[rate[code][0][0]] = strategy[rate[code][0][0]] + 1 #best strategy
    ...
    profit_all = pd.concat([based_profit, best_strategy_profit], axis=1).fillna(method='bfill')
    '''
    plt.plot(profit_all)
    plt.legend(('based_profit', 'best_strategy_profit'), loc='center')
    plt.title(code)
