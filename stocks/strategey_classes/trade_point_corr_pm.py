...
class GetTradePoint(GP):
    def __init__(self,df,start_date,end_date,norm_type='max',quota_index=0,lost=1.0):
        super(GetTradePoint,self).__init__(df,start_date,end_date,norm_type='max',quota_index=0,lost=1.0)
        self.__quotas = GetQuotas(self.__df).quotas

    def get_sign_score(self,quotas,close='',types=('pm_buy','buy')):
        r1 = self.get_quotas_signs(quotas[0].T,types=types[0])
        r2 = self.get_quotas_signs(quotas[2].T,types=types[1])
        r = r1.T+r2.T
        corr = quotas[0].dropna().corr()[['close']]
        for k in list(corr.index):
            r[k] = r[k]*corr.loc[k]['close']
        rr = pd.DataFrame(r.T.drop(['J','volume']).sum())
        rr.columns = ['sign']
        return rr.dropna()
    ...
    def get_quotas_signs(self,quotas='',types=''):
        ...
        quotas.apply(self.__get_quotas_cmp_sign,types=types)
    ...
    def get_trade_point(self,quotas=''):
        ...
        signs_buy = self.get_sign_score(quotas)
        types=('pm_sell','sell')
        signs_sell = self.get_sign_score(quotas,types = types)
        signs = signs_buy - signs_sell
