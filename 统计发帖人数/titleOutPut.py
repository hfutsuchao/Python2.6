...
geren = open('new.txt','w')

def getSql(category,datetime,type):
    if type == 'all':
        sql = 'SELECT title FROM '+category+' WHERE listing_status =5 and show_time between unix_timestamp("'+datetime+'") and unix_timestamp("'+datetime+'")+7*86400 ;'
    else:
...
for domain in citys:
    ...
    for category in categorys:
        #print category
        sqlGeren = getSql(category,'2013-12-18',"all")
        ...
        for row in cursor.fetchall():
            ...
            geren.write(category+'\t'+str(row[0])+'\n')
